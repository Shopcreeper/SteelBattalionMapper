...
internal sealed class KeyboardMouseOutput : IDisposable
{
    ...
    private double _smoothedMouseX;
    private double _smoothedMouseY;
    ...
    public void MoveMouse(double normalizedX, double normalizedY, double sensitivity = 1.0)
    {
        ...
        const double accelerationTimeSeconds = 0.10;
        ...
        if (Math.Abs(targetX) < microscopicVelocity)
            targetX = 0;

        if (Math.Abs(targetY) < microscopicVelocity)
            targetY = 0;
        ...
        double dt =
            _lastMouseMoveTick == 0
                ? 0.016
                : Math.Clamp((now - _lastMouseMoveTick) / 1000.0, 0.001, 0.050);

        _lastMouseMoveTick = now;

        // Smooth acceleration toward greater motion, but do NOT smooth
        // deceleration. Returning toward center or reversing direction responds
        // immediately so aiming never feels floaty.
        _smoothedMouseX = SmoothAccelerationOnly(
            _smoothedMouseX,
            targetX,
            dt,
            accelerationTimeSeconds);

        _smoothedMouseY = SmoothAccelerationOnly(
            _smoothedMouseY,
            targetY,
            dt,
            accelerationTimeSeconds);
        ...
        {
            _mouseRemainderX += _smoothedMouseX;
        }
        ...
        {
            _mouseRemainderY += _smoothedMouseY;
        }
    ...
    private static double SmoothAccelerationOnly(
        double current,
        double target,
        double dt,
        double accelerationTimeSeconds)
    {
        ...
        double alpha =
            1.0 - Math.Exp(-dt / Math.Max(0.001, accelerationTimeSeconds));
